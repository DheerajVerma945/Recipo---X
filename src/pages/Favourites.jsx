...
import { getFavourites, clearFavourites } from '../appwriteService/user';
...
function Favourites() {
  ...
  useEffect(() => {
    const fetchFavourites = async () => {
      ...
      try {
        const favIds = await getFavourites(userId);
  ...
  const handleClearAll = async () => {
    try {
      setAlertMessage('All favourites have been cleared.');
      await clearFavourites(userId);
      setData([]);
      window.location.reload;
    } catch (error) {
      setError('Failed to clear favourites.');
    } finally {
  ...
  if (data.length === 0) {
    return <div className='flex items-center justify-center h-screen'>
      <p className="text-center text-lg">No favourites found.</p>
    </div>;
  }
  ...
  return (
    <div className="pt-40 w-full  mx-auto bg-white shadow-md rounded-lg p-4 relative">
      {showClearConfirm && (
        <div className="absolute inset-0 w-full bg-gray-800 bg-opacity-50 flex items-center justify-center z-10">
          <div className="bg-white p-6 rounded-lg shadow-lg relative w-11/12 max-w-sm">
            <h2 className="text-lg font-bold mb-4">Are you sure you want to clear all favourites?</h2>
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Your Favourites <span className='text-xl  font-semibold'>({data.length})</span></h1>
        <button
